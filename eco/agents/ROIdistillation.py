...
class ROIdistillation(BaseAgent):
    ...
    def __init__(self, ):
        self.agent = create_agent(Config.chatOpenAI, response_format=ROIdisstillationModel)
    ...
    async def embeddingROI(self, roi: ROIdisstillationModel, metadata: Data.TrainHQ):
        document = Document(page_content=roi.model_dump_json(), metadata=metadata)
        doc_id = f"{metadata.src_id}-{metadata.tgt_id}"
        self.vector_store.add_documents(documents=[document], ids=[doc_id])
        return document, doc_id

    async def batchEmbeddingROI(self, rois: list[ROIdisstillationModel], metadatas: list[Data.TrainHQ]):
        if len(rois) != len(metadatas):
            return
        documents = []
        doc_ids = []
        for i in range(0, len(rois)):
            document = Document(page_content=rois[i].model_dump_json(), metadata=metadatas[i])
